...
# Defina o shell apenas uma vez no topo (opcional, mas recomendado)
SHELL ["/bin/bash", "-o", "pipefail", "-c"]

RUN dnf update -y && \
    dnf copr enable -y alternateved/eza && \
    dnf install -y \
        ncurses wget curl libicu gnupg2 openssh-clients \
        iputils bind-utils busybox eza fzf unzip ansible \
